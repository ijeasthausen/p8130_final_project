...
In order to improve residual normality, observations with outlier residual values (>2.5 after standardization) were removed from the data, and the model was re-fit using the process described above. After outliers were removed from the data, backward, forwards, and stepwise variable processes all chose the following variables for inclusion in the model: c-index, number of ER visits in last six months, age, gender, marital status, insurance type, BMI, temperature, heart rate, respiration rate, and mean arterial pressure. Additional variables chosen by the forward selection process include: 30 day readmit, ICU, and religion. The backward process also included 30 day readmit, and the stepwise process did not select any additional variables. All three models performed similarly in model diagnostic criteria (table XXX). An ANOVA test to compare nested models was used to make all pairwise comparisons between candidate models. 
